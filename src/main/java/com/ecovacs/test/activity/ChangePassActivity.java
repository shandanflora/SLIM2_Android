...
public class ChangePassActivity {
    ...
    private boolean samePassword(Map<String, String> tranMap){
        ...
        Common.getInstance().goBack(driver, 1);
        btnSavePass.click();
        boolean btextMessage = textMessage.getText().equalsIgnoreCase(tranMap.get("oldpassword_newpassword_same"));
        if (!btextMessage){
            TranslateErrorReport.getInstance().insetNewLine(
                    tranMap.get("language"), "ChangePassword", textMessage.getText(),
                    tranMap.get("oldpassword_newpassword_same"), "fail");
        }
        return btextMessage;
    }

    private boolean wrongOldPassword(Map<String, String> tranMap){
        editOldPass.clear();
        editOldPass.sendKeys("123456kk");
        Common.getInstance().goBack(driver, 1);
        editNewPass.clear();
        editNewPass.sendKeys("123456qq");
        Common.getInstance().goBack(driver, 1);
        editRePass.clear();
        editRePass.sendKeys("123456qq");
        Common.getInstance().goBack(driver, 1);
        btnSavePass.click();
        showActivity();
        boolean btextMessage = textMessage.getText().equalsIgnoreCase(tranMap.get("wrong_old_password"));
        if (!btextMessage){
            TranslateErrorReport.getInstance().insetNewLine(
                    tranMap.get("language"), "ChangePassword", textMessage.getText(),
                    tranMap.get("wrong_old_password"), "fail");
        }
        return btextMessage;
    }
