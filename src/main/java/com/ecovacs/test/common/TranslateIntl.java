...
/*
    ...
    public int getColNum(Sheet sheet, String strLanguage){
        int iColSize = sheet.getRow(0).getLastCellNum();
        int iColIndex = -1;

        for(int i = 0; i < iColSize; i++){
            if(getCellValue(sheet.getRow(0).getCell(i)).trim().equals(strLanguage)){
                iColIndex = i;
                break;
            }
        }
    ...
    public Map<String, String> readExcel(String strFile, String strColName){
        ...
        try{
            ...
            for(int i = 1; i <= iRowSize; i++){//headline(i = 0)
                ...
                String strValue = getCellValue(sheet.getRow(i).getCell(0));
